...
def run_doctor(model, ram_gb=0, context=4096, gpu_indices=None, vram_gb=0, *,
    ...
    # Phase 1: hard memory ceiling backend availability (cgroup / Job Object / rlimit).
    hard_off = os.environ.get("COLI_HARD_CAP", "1") == "0"
    if hard_off:
        checks.append(_check("memory.hard_cap", "warn",
                             "COLI_HARD_CAP=0: kernel memory ceiling disabled"))
    elif sys.platform == "linux":
        checks.append(_check("memory.hard_cap", "pass",
                             "hard cap backend: cgroup v2 memory.max (RLIMIT_AS fallback)"))
    elif sys.platform == "win32":
        checks.append(_check("memory.hard_cap", "pass",
                             "hard cap backend: Windows Job Object process memory limit"))
    elif sys.platform == "darwin":
        checks.append(_check("memory.hard_cap", "pass",
                             "hard cap backend: macOS footprint limit or soft_evict equivalent"))
    else:
        checks.append(_check("memory.hard_cap", "warn",
                             f"hard cap backend unknown on platform {sys.platform}"))
