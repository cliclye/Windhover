...
- **Fast / Balanced / Low-RAM profiles** actually set `RAM_GB` (AU on when capped), `MLOCK`, and `WH_SPARSE`. Default **Balanced** on machines with ≤16 GB (~55% of RAM). Chat shows the cap and a profile picker.
