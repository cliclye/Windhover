    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-row">
          <div className="brand-mark"><Feather className="size-4" /></div>
          <div><h1>Windhover</h1><p>Local engine</p></div>
        </div>
        ...
        <nav className="side-nav" aria-label="Views">
          ...
          <button type="button" className={view === "brain" ? "active" : ""} onClick={() => setView("brain")}>
            Brain
          </button>
        </nav>
            ...
            <div className="session-list">
              {Array.from({ length: kvSlots }, (_, slot) => (
                <button
                  key={slot}
                  type="button"
                  className={cacheSlot === slot ? "active" : ""}
                  disabled={loading}
                  onClick={() => setCacheSlot(slot)}
                >
                  Session {slot + 1}
                  {(conversations[slot] || []).length ? ` · ${(conversations[slot] || []).length}` : ""}
                </button>
              ))}
            </div>
          </section>
        ...
        <details className="side-details" open={settingsOpen} onToggle={(e) => setSettingsOpen(e.currentTarget.open)}>
          <summary>Settings</summary>
          <section className="side-section">
            <label>API endpoint<Input value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label>
            <label>API key<div className="relative"><KeyRound className="field-icon" /><Input className="pl-9" type="password" value={apiKey} placeholder="optional" onChange={(event) => setApiKey(event.target.value)} /></div><span className="field-help">Kept in memory only</span></label>
          ...
          <section className="side-section runtime-section" aria-live="polite">
            ...
            {health?.scheduler ? <>
              ...
              {totalTokens.prompt + totalTokens.completion > 0 ? <div className="session-stats">
                <span>Session: <strong>{totalTokens.prompt.toLocaleString()}</strong> prompt + <strong>{totalTokens.completion.toLocaleString()}</strong> completion</span>
              </div> : null}
        ...
        <div className="sidebar-foot">OpenAI-compatible transport</div>
      </aside>

      <main className="chat-panel">
        <header className="topbar">
          <div><span className="eyebrow">Model</span><strong>{model}</strong></div>
          <div className="top-actions">
            {loading && tokenCount > 0 ? <Badge className="badge-live">{tokenCount} tokens</Badge> : null}
            {!loading && tokPerSec != null ? <Badge>{tokPerSec.toFixed(1)} tok/s</Badge> : null}
        ...
        <div className="conversation">
          {!messages.length ? (
            <div className="empty-state">
              <h2>Ask anything.</h2>
              <p>Connect to a local Windhover server. Nothing leaves the endpoint you choose.</p>
              <div className="suggestions">
                {["Explain how expert routing works", "Write a small C benchmark", "Compare RAM and VRAM caching"].map((item) => <button key={item} onClick={() => setDraft(item)}>{item}</button>)}
              </div>
            </div>
          ) : (
            <div className="message-list">
              {messages.map((item) => (
                <article key={item.id} className={cn("message", item.role)}>
                  <div className="message-meta">{item.role === "user" ? "You" : "Windhover"}</div>
                  <div className="message-body">{item.content || <span className="typing" aria-label="Generating"><i /><i /><i /></span>}</div>
                </article>
              ))}
              <div ref={bottomRef} />
            </div>
