...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    /* EMBED_STREAM: default OFF (fast path). Budget path / user can set EMBED_STREAM=1
     * when RAM-starved — lossless page-in of embedding rows. */
    g_embed_stream = getenv("EMBED_STREAM") ? atoi(getenv("EMBED_STREAM")) : 0;
    if(g_embed_stream){
...
#endif
    /* Kestrel: auto-absorb up to S<=32 (stock S<=4). Bit-identical on glm_tiny TF;
     * skips rebuilding full K/V via kv_b for moderate prefills. */
    int absorb = kvs || g_absorb==1 || (g_absorb<0 && S<=32) || cuda_absorb;
...
static void forward_all(Model *m, const int *ids, int S, int *pred){
    ...
    /* Kestrel: one batched lm_head matmul for all positions (stock does S×S=1). */
    float *normed=falloc((int64_t)S*D);
    #pragma omp parallel for schedule(static)
    for(int s=0;s<S;s++)
        rmsnorm(normed+(int64_t)s*D, x+(int64_t)s*D, m->final_norm, D, c->eps);
    float *lo=falloc((int64_t)S*V);
...
int main(int argc, char **argv){
    ...
    g_absorb = getenv("ABSORB")?atoi(getenv("ABSORB")):-1; /* -1 auto: absorb for S<=32 (Kestrel; stock was S<=4) */
    ...
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      { double bud = ram_env;
        ...
        /* Skip hard-cap arming in TF/oracle mode unless explicitly requested —
         * footprint probes + logging dominated multi-process benches. */
        int want_cap = 1;
        if(getenv("COLI_HARD_CAP")) want_cap = atoi(getenv("COLI_HARD_CAP"));
        else if(getenv("TF")) want_cap = 0;
