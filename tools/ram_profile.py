...
Default is Balanced on machines with ≤16 GB physical RAM so laptop Chat
actually hits the residency ceiling (Library currently left RAM_GB unset).
"""
...
def physical_ram_gb() -> float:
    """Installed physical RAM in GB (not free/available). 0 if unknown."""
    global _PHYS_CACHE
    ...
    if sys.platform == "win32":
        try:
            ...
            if kernel32.GetPhysicallyInstalledSystemMemory(ctypes.byref(total_kb)) and total_kb.value:
                return (total_kb.value * 1024) / 1e9
