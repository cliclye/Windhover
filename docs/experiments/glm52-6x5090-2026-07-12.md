...
This note records the July 12, 2026 experiments that compared the upstream engine with
...
The day had two halves. The morning runs (below) measured the upstream engine at 0.12
tok/s and vLLM-Moet at 2.3-2.7 tok/s. The afternoon full-resident work on
the upstream engine (see "full-resident experiments") removed disk from the decode
...
- upstream INT4 model: `/data/models/GLM-5.2-int4` (144 shards)
...
| --- | --- | ---: | ---: | --- |
| upstream INT4 (morning, partial residency) | 6 GPUs | about 42 s | 0.12 tok/s | GPU hot tier was active, but dense compute remained on CPU |
| upstream INT4 (afternoon, full residency) | 6 GPUs + 24 cores | — | 6.28-6.84 tok/s | all 19,456 experts resident in VRAM+RAM, zero disk waits |
...
The morning upstream run generated 16 tokens in 132.46 seconds. Its measured
...
## full-resident experiments (afternoon)

Test directory: `/data/test/full-resident`. Related PR:
[#80](https://github.com/cliclye/Windhover/pull/80) (on top of PR #68). All A/B
...
(`mateogrgic/GLM-5.2-int4-with-int8-mtp`, ~10 GB; originals kept as
...
Logs: `/data/test/six-gpu-ab-mtp-*-20260712.log` (phase 1),
`/data/test/six-gpu-ab-mtp8-*-20260713.log` (phase 2).
...
`/data/test/six-gpu-ab-avx512q-*-20260713.log`,
...
## Conclusions for the upstream engine
...
model fits in VRAM+RAM, and once disk left the decode path the upstream engine at
6.28-6.84 tok/s beat every vLLM-Moet layout measured here (best 2.6 tok/s),
because the upstream engine never replays a whole step on an expert miss.
...
The fastest configuration overall is the upstream full-resident setup in
`/data/test/full-resident` (6.28-6.84 tok/s; stop the
...
reproduced. Raw A/B logs live under `/data/test/six-gpu-ab-*.log`.
